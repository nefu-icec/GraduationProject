...
$('.newGroup').on('click', 'button', function(){
	...
	if(cptNum - iValue != 0){
		...
		$.get('StartServlet?task=firstGroup&group=' + subValue, function(data, status){
			//do something......
			if(data > 0)
			{
				checkSystemStatus();
				alert('分配成功!');
				$('#content').load('start/subgroup.jsp');
			}
			else
				alert('分配失败,请重试!');
		});
...
$('.glyphicon-cloud-download').click(function(){
	...
	$.get('StartServlet?task=createExcel&type=firstGroup&group=' + groupId, function(data, status){
		if(data == 'true'){
...
$('.switch-mini').on('switch-change', function (e, data) {
	...
	$.get("StartServlet?task=setStartPassed&sid="+sId+"&passed="+isPassed,null, function(data){
		if(data == 'true'){
			checkSystemStatus();
			$this.next('span').hide();
		}else{
...
$('.chge-yes').click(function(){
	...
	$.post('DataServlet?task=modify', data, function(data, status){
		if(data == 'true'){
			...
			checkSystemStatus();
			var tblNum = tblIdx;
			$('#content').load('start/subgroup.jsp',function(){
				$('.nav-tabs li').removeClass('active');
				$('.showInfo').removeClass('show');
