...
$('.newGroup').on('click', 'button', function(){
	...
	if(cptNum - iValue != 0){
		...
		$.get('StartServlet?task=secondGroup&group=' + subValue, function(data, status){
			//do something......
			if(data > 0)
			{
				checkSystemStatus();
				alert('分配成功!');
				$('#content').load('start/sedSubgroup.jsp');
			}else
				alert('分配失败,请重试!');
		});
	}
...
$('.glyphicon-cloud-download').click(function(){
	...
	$.get('StartServlet?task=createExcel&type=secondGroup&group=' + groupId, function(data, status){
		if(data == 'true'){
...
$('.chge-yes').click(function(){
	...
	$.post('DataServlet?task=modify', data, function(data, status){
		if(data == 'true'){
			...
			checkSystemStatus();
			var tblNum = tblIdx;
			$('#content').load('start/sedSubgroup.jsp',function(){
				$('.nav-tabs li').removeClass('active');
				$('.showInfo').removeClass('show');
